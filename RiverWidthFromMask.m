...
for i = 1:n
    ...
    if ~isempty(tokens)
        years(i) = str2double(tokens{end});   % take LAST number (the year)
...
for i = 1:(n-1)
    ...
    % I1 = riv(i).im.hc;   % full channel mask (original, with islands)
    % I2 = riv(i+1).im.hc;
...
%% Export total maps as GeoTIFF for ArcGIS (FIXED WITH CRS)
