...
for i = 1:n
    ...
    I = bwareaopen(I, 100);
     ...
     % --- Compute dynamic Wn from mask ---
D = bwdist(~I);              % distance to bank
width_map = 2 * D;           % full width at each pixel

% Extract widths only inside channel
channel_widths = width_map(I);

% Remove extreme values (optional but recommended)
channel_widths = channel_widths(channel_widths > 0);
channel_widths = channel_widths(~isnan(channel_widths));

% Robust estimate (median is better than mean) 

Wmed = median(channel_widths);

Wn_dynamic = round(0.4 * Wmed);

% Safety bounds (adjusted)
Wn_dynamic = max(round(0.25 * Wmed), min(Wn_dynamic, 80));

Wn = double(Wn_dynamic);    % nominal channel width (pixels)
    
    % Braiding index on cleaned (multi‑thread) mask
    CC = bwconncomp(I);
    braiding_index = CC.NumObjects;
    % Keep full channel (IMPORTANT for cutoffs)
     riv(i).im.hc = I;

% Extract single-thread ONLY for centerline
...
% [Wm,SWm] = width_from_mask(Ist,cl,spacing);
 % --- Width from mask (FIXED VERSION) ---
Wn_width = round(0.5 * Wavg);   % larger scale for stability
spacing = Wn_width;

try
    [Wm, SWm] = width_from_mask(Ist, cl, spacing);
catch
    warning('width_from_mask failed (single year)');
    Wm = [];
    SWm = [];
end
...
figure
plot(S, Wbl, 'b', 'LineWidth', 1.5); hold on

if ~isempty(Wm)
    plot(SWm, Wm, 'r', 'LineWidth', 1.5);
end

legend('Bankline Width','Mask Width')
xlabel('Streamwise distance')
ylabel('Width (pixels)')
title(['Width Comparison - ' num2str(riv(i).meta.year)])
grid on
...
for i = 1:numel(riv)
    ...
    Wavg = mean(W(~isnan(W)));
    fprintf('Year %d → Wn: %.2f px | Wavg: %.2f px\n', riv(i).meta.year, Wn, Wavg);
    ...
    % --- Width from mask (FIXED VERSION) ---
Wn_width = round(0.5 * Wavg);
spacing = Wn_width;

try
    [Wm, SWm] = width_from_mask(I, cl, spacing);
catch
    warning('width_from_mask failed for year %d', riv(i).meta.year);
    Wm = [];
    SWm = [];
end
...
for i = 1:numel(riv)-1
    if isfield(riv(i).mig.cl, 'cutarea')
        fprintf('Year %d → %d cutoffs\n', ...
            riv(i).meta.year, length(riv(i).mig.cl.cutarea));
    else
        fprintf('Year %d → NO cutoffs detected\n', riv(i).meta.year);
    end
end
